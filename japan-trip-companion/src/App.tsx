...
import { clockInJapan, isoInJapan, minutesInJapan, resolveActiveDate } from './utils/date'
...
export default function App() {
  ...
  const stickyDay = tab === 'trip' && openDate ? days.find((d) => d.date === openDate) : activeDay
    ...
    <div className="mx-auto flex min-h-dvh max-w-2xl flex-col bg-paper sm:border-x sm:border-sumi-100">
      ...
      <header className="sticky top-0 z-30 border-b border-sumi-100 bg-paper/85 backdrop-blur-xl">
        <div className="flex items-center justify-between gap-3 px-5 py-3">
          {stickyDay ? <DayHeadline day={stickyDay} /> : <span className="eyebrow">Japan 2026</span>}
      ...
      <main className="flex-1 px-5 pt-6 pb-[calc(76px+env(safe-area-inset-bottom))]">
        {tab === 'today' && (
          <TodayView state={state} today={today} now={now} nowMinutes={nowMinutes} />
        )}
        {tab === 'trip' && (
          <TripView
            state={state}
